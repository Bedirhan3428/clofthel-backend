...
const CARD_HEIGHT = 146; // 16:9 aspect ratio roughly
...
function AnimeLandscapeCard({ item, onPress }) {
  const imageSource = item.bannerImage || item.banner_image || item.coverImage;
  ...
  return (
    ...
    >
      ...
      <View style={styles.card}>
        ...
        {/* Episode/Year badge (Bottom Left) */}
        {episodesCount ? (
          ...
          <View style={styles.episodeBadge}>
            <Text style={styles.episodeText}>{item.seasonYear}</Text>
          </View>
        ) : null}
      </View>

      {/* ── Title and Snippet below card ───────────── */}
      ...
      {item.description ? (
        ...
        <Text style={styles.cardDescription} numberOfLines={1}>
          {(item.enrichedGenres || item.genres).slice(0, 3).join(' • ')}
        </Text>
      ) : null}
    </TouchableOpacity>
  );
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          windowSize={3}
          renderItem={({ item }) => (
            <AnimeLandscapeCard item={item} onPress={onAnimePress} />
          )}
        />
      )}
    </View>
